import * as React from "react";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { cn } from "@/lib/utils";

const defaultColorCombinations = [
  {
    start: "hsl(220, 15%, 8%)",
    middle: "hsl(200, 12%, 18%)",
    end: "hsl(240, 10%, 12%)",
  },
  {
    start: "hsl(35, 25%, 15%)",
    middle: "hsl(30, 20%, 25%)",
    end: "hsl(25, 15%, 10%)",
  },
  {
    start: "hsl(210, 20%, 12%)",
    middle: "hsl(200, 15%, 22%)",
    end: "hsl(215, 18%, 8%)",
  },
  {
    start: "hsl(150, 30%, 8%)",
    middle: "hsl(140, 25%, 15%)",
    end: "hsl(160, 20%, 6%)",
  },
  {
    start: "hsl(230, 35%, 6%)",
    middle: "hsl(220, 30%, 12%)",
    end: "hsl(240, 25%, 4%)",
  },
  {
    start: "hsl(20, 40%, 12%)",
    middle: "hsl(15, 35%, 18%)",
    end: "hsl(10, 30%, 8%)",
  },
  {
    start: "hsl(280, 25%, 10%)",
    middle: "hsl(270, 20%, 16%)",
    end: "hsl(290, 15%, 6%)",
  },
  { start: "hsl(0, 0%, 8%)", middle: "hsl(0, 0%, 18%)", end: "hsl(0, 0%, 4%)" },
];

const hashString = (str) => {
  let hash = 0;
  for (let i = 0; i < str.length; i++) {
    const char = str.charCodeAt(i);
    hash = (hash * 31 + char) % 0x1_00_00_00_00;
  }
  return hash;
};

const SVGPattern = React.memo(({ colors, uniqueId, randomSeed }) => {
  const gradientId = `ffflux-gradient-${uniqueId}`;
  const filterId = `ffflux-filter-${uniqueId}`;

  return (
    <svg
      aria-label="Loading pattern"
      height="100%"
      preserveAspectRatio="xMidYMid slice"
      role="img"
      version="1.1"
      viewBox="0 0 100 100"
      width="100%"
      xmlns="http://www.w3.org/2000/svg"
    >
      <defs>
        <linearGradient
          gradientTransform="rotate(150, 0.5, 0.5)"
          id={gradientId}
          x1="50%"
          x2="50%"
          y1="0%"
          y2="100%"
        >
          <stop offset="0%" stopColor={colors.start} stopOpacity="1" />
          {colors.middle && (
            <stop offset="50%" stopColor={colors.middle} stopOpacity="1" />
          )}
          <stop offset="100%" stopColor={colors.end} stopOpacity="1" />
        </linearGradient>
        <filter
          colorInterpolationFilters="sRGB"
          filterUnits="objectBoundingBox"
          height="140%"
          id={filterId}
          primitiveUnits="userSpaceOnUse"
          width="140%"
          x="-20%"
          y="-20%"
        >
          <feTurbulence
            baseFrequency="0.005 0.003"
            height="100%"
            numOctaves={2}
            result="turbulence"
            seed={randomSeed}
            stitchTiles="stitch"
            type="fractalNoise"
            width="100%"
            x="0%"
            y="0%"
          />
          <feGaussianBlur
            edgeMode="duplicate"
            height="100%"
            in="turbulence"
            result="blur"
            stdDeviation="20 0"
            width="100%"
            x="0%"
            y="0%"
          />
          <feBlend
            height="100%"
            in="SourceGraphic"
            in2="blur"
            mode="color-dodge"
            result="blend"
            width="100%"
            x="0%"
            y="0%"
          />
        </filter>
      </defs>
      <rect
        fill={`url(#${gradientId})`}
        filter={`url(#${filterId})`}
        height="100%"
        width="100%"
      />
    </svg>
  );
});

SVGPattern.displayName = "SVGPattern";

export function ImageLoader({
  src,
  alt,
  className,
  blurIntensity = "blur(50px)",
  width = 400,
  height = 300,
  fallbackComponent,
  onLoad,
  onError,
  loading = "lazy",
  objectFit = "cover",
  customColors,
  seed,
  loaderColor, // New prop for solid background color loader
}) {
  const [isLoaded, setIsLoaded] = useState(false);
  const [hasError, setHasError] = useState(false);
  const imgRef = useRef(null);

  useEffect(() => {
    setIsLoaded(false);
    setHasError(false);
  }, []);

  const patternData = useMemo(() => {
    // If loaderColor is provided, create a simple solid color configuration
    if (loaderColor) {
      return {
        useSimpleLoader: true,
        color: loaderColor,
        uniqueId: `${hashString(src || "default")}-simple`.slice(0, 9),
      };
    }

    const colorCombinations =
      customColors && customColors.length > 0
        ? customColors
        : defaultColorCombinations;

    const seedValue = seed !== undefined ? seed : hashString(src || "default");
    const colorIndex = seedValue % colorCombinations.length;
    const selectedColors = colorCombinations[colorIndex];

    const randomSeed = seedValue % 1000;
    const uniqueId = `${hashString(src || "default")}-${seedValue}`.slice(0, 9);

    return {
      useSimpleLoader: false,
      colors: selectedColors,
      uniqueId,
      randomSeed,
    };
  }, [customColors, seed, src, loaderColor]);

  const handleLoad = useCallback(() => {
    setIsLoaded(true);
    onLoad?.();
  }, [onLoad]);

  const handleError = useCallback(
    (event) => {
      setHasError(true);
      setIsLoaded(true);
      onError?.(event.nativeEvent);
    },
    [onError]
  );

  const handleImageRef = useCallback((img) => {
    if (img?.complete && img?.naturalHeight !== 0) {
      setIsLoaded(true);
    }
  }, []);

  const containerStyle = useMemo(
    () => ({
      width: typeof width === "number" ? `${width}px` : width,
      height: typeof height === "number" ? `${height}px` : height,
    }),
    [width, height]
  );

  return (
    <div
      aria-label={alt}
      className={cn("relative overflow-hidden rounded-lg", className)}
      role="img"
      style={containerStyle}
    >
      {!hasError && (
        <div
          aria-hidden="true"
          className={cn(
            "absolute inset-0 transition-opacity duration-500",
            isLoaded ? "opacity-0" : "opacity-100"
          )}
          style={{
            filter: patternData.useSimpleLoader ? "none" : blurIntensity,
            backgroundColor: patternData.useSimpleLoader
              ? patternData.color
              : undefined,
          }}
        >
          {!patternData.useSimpleLoader && (
            <SVGPattern
              colors={patternData.colors}
              randomSeed={patternData.randomSeed}
              uniqueId={patternData.uniqueId}
            />
          )}
        </div>
      )}
      {!hasError && (
        <img
          alt={alt}
          className={cn(
            "absolute inset-0 h-full w-full transition-opacity duration-500",
            isLoaded ? "opacity-100" : "opacity-0"
          )}
          decoding="async"
          height={height}
          loading={loading}
          onError={handleError}
          onLoad={handleLoad}
          ref={(img) => {
            if (imgRef.current) {
              imgRef.current = img;
            }
            handleImageRef(img);
          }}
          src={src}
          style={{ objectFit }}
          width={width}
        />
      )}
      {hasError && (
        <div className="absolute inset-0 z-10 flex items-center justify-center bg-muted">
          {fallbackComponent ? (
            fallbackComponent
          ) : (
            <div className="text-center text-muted-foreground">
              <svg
                aria-hidden="true"
                className="mx-auto mb-2 h-12 w-12"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
                />
              </svg>
              <p className="text-sm">Failed to load image</p>
            </div>
          )}
        </div>
      )}
    </div>
  );
}
